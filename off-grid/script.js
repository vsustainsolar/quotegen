...
function buildSummaryQuotationHtml(totals, systemType) {
  // Generate detailed quote HTML first
  let html = buildDetailedQuotationHtml(totals, systemType);
  
  // Replace individual item prices with "-" in the commercial table
  // Match pattern: <td class="p-3 border text-right">₹X,XXX.XX</td> for line items
  html = html.replace(
